...
use embassy_stm32::i2c::{self, Config as I2cConfig, I2c, SlaveAddrConfig, SlaveCommandKind, Smbus, SmbusConfig};
...
const EXPORT_SLAVE_ADDR: u8 = 0x42;
...
async fn sensor_master_task(mut master: Smbus<'static, embassy_stm32::mode::Blocking, i2c::mode::Master>) {
    let reg = [0u8];
...
#[embassy_executor::task]
async fn export_slave_task(mut slave: Smbus<'static, embassy_stm32::mode::Blocking, i2c::mode::MultiMaster>) {
    loop {
            ...
            Ok(cmd) => match cmd.kind {
                SlaveCommandKind::Read => {
                    ...
                    match slave.blocking_respond_to_read(&payload) {
                        Ok(n) => info!("export read: sent {} bytes {:02X}{:02X}", n, payload[0], payload[1]),
